...
This is the core loop of using Prompt Lockbox. This guide will walk you through the entire lifecycle of a single prompt: from its initial creation as a draft, to securing it for production, and finally to creating a new version for safe iteration.
...
<Steps>
  ...
  <Step title="Edit and Finalize">
    Now, open the newly created `prompts/joke-generator.v1.0.0.yml` file and add your template logic. This is the only manual step in the process.
...
<Check>
  You have now mastered the core lifecycle of a prompt: creating it, securing it for production with lock, and safely iterating on it with version. This workflow is fundamental to maintaining a reliable and organized prompt library.

  **Next up**: [Learn how to find and navigate your growing collection of prompts](/second).
</Check>