...
This guide will walk you through setting up LLMs for use within the toolkit. Whether you're connecting to a popular **cloud-based provider**, **running a local model**, or **integrating your own custom model** (checkout list at [supported models](/ai_features#supported-models)). Follow the steps below to get started with the setup that best fits your workflow.
