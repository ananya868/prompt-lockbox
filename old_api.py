...
class Prompt:
    ...
    def __init__(self, path: Path, data: Dict[str, Any], project_root: Path):
        self.path = path
        self.data = data
        # Store the whole project object to access its config
        self._project = project
        # Keep a direct reference to the root for convenience in other methods
    ...
    def document(self):
        ...
        print(f"📄 Analyzing prompt '{self.name}' to generate documentation...")
        template_content = self.data.get("template", "")
        if not template_content: raise ValueError("Cannot document an empty prompt.")

        # Get the dynamic AI config from the project object
        ai_config = self._project.get_ai_config()
        
        # Call the AI engine with the template and the config
        new_docs = proompt_documenter.get_documentation(
            template_content, 
...
class Project:
    ...
    def __init__(self, path: str | Path | None = None):
        ...
        # Load the config once during initialization for efficiency
    ...
    def get_ai_config(self) -> dict:
        """
        Returns the AI configuration from plb.toml, providing defaults if not set.
        """
        # Get the [ai] section from the loaded config   
        ai_config = self._config.get("ai", {})
        
        # Return a dictionary with defaults applied
        return {
    ...
    def create_prompt(
        ...
        return Prompt(path=filepath, data=data_from_content, project_root=self)

    # --- END OF NEW METHOD ---
    
    def get_prompt(self, identifier: str) -> Optional[Prompt]:
        """Finds and loads a prompt by name or path."""
        ...
        # FIX: Pass `project=self` instead of `project_root=self.root`
        return Prompt(path=prompt_path, data=prompt_data, project=self)
    ...
    def list_prompts(self) -> List[Prompt]:
        ...
        for path in all_prompt_paths:
            ...
            if data:
                prompts.append(Prompt(path=path, data=data, project_root=self))
        return prompts
